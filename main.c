...
CFG:
  ...
  memory management:
    ...
    memory table:
      ...
      |                |                  |                   |
      | index : uint64 | address : uint64 | byte size :uint64 |
      |________________|__________________|___________________|

    variable memory mapping:
      ________________________________________________
      |                |                             |
      | var : Variable | memory table index : uint64 |
      |________________|_____________________________|

    I have more types of memory dereferencing statement:

      variable deref : variable --> memtable index
        syntax: | variable |

      memtable offset deref : memtable index X offset --> memory piece
        syntax: < memtable idx : offset >

      memtable direct deref : variable --> memory piece
        syntax: < variable >
    ...
    CFG CODE:
      - decl s :: S
      - decl m.0 :: MemtableIdx
      - < m.0 > = | s |
      - decl m.1 :: MemtableOffset
      - < m.1 > = RelativePos(p, S)
      - < m.0, m.2 > = 10
  ...
  CFGStatement:
    ...
    - Destruction:
        undecl [variable name]
    
    - Assignment: 
        [memory piece] := [memory piece]    

  CFGExpression:

    - MemTableDirectAccess(Variable) --> MemoryPiece:
        < [variable] >

    - MemTableOffsetAcces(MemTableIndex, MemoryOffset) --> MemoryPiece:
        < [memtable index], [offset] >

    - VarTableAcces(Variable) --> MemTableIndex:
        < [variable] >

    - FunctionCall(Function, [MemoryPiece, MemoryPiece, ...]):
        [function name]([memory piece], [memory piece], ...)
  ...
  CFGDataType:
    ...
    VariableMemoryMapping = {
      Map<Variable, MemTableIndex> var_table;
    }

    MemoryTable = {
      Table<MemTableIndex, Address, ByteSize> mem_table;

      MemTableIndex Alloc(ByteSize);
      MemoryPiece Access(MemTableIndex, Offset);
    }
